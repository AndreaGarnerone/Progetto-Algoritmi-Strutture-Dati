...
public class Graph<V, L> implements AbstractGraph<V, L> {
    ...
    /**
     * Retrieves a collection of the neighbours vertex for a specified vertex
     *
     * @param a the reference vertex for which to find the neighbors
     * @return A collection of all the neighbours vertex of a
     */
    ...
    public L getLabel(V a, V b) {
        ...
        if (containsEdge(a, b)) {
            boolean found = false;
            for (int i = 0; !found && i < map.get(a).size(); i++) {
                found = b.equals(map.get(a).get(i).getEnd());
                if (found) label = map.get(a).get(i).getLabel();
            }
        }
        return label;
    } // getLabel
